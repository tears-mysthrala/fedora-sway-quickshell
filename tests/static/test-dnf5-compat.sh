...
grep -Fq 'dnf "${DNF_REPO_ARGS[@]}" -q makecache >/dev/null' "$ROOT/install.sh"
...
grep -Fq "DNF_REPO_ARGS=(--disablerepo='*'" "$ROOT/install.sh"
grep -Fq 'dnf "${DNF_REPO_ARGS[@]}"' "$ROOT/install.sh"
