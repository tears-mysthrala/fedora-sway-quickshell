...
! grep -RniE '\b(setenforce[[:space:]]+0|SELINUX=disabled|chmod[[:space:]]+-R[[:space:]]+777|curl[^|]*\|[[:space:]]*(ba)?sh)\b' "$ROOT" --exclude-dir=.git --exclude-dir=.vm --exclude-dir=.evidence --exclude=run.sh
