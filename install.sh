...
DNF_REPO_ARGS=(--disablerepo='*' --enablerepo=fedora --enablerepo=updates --enablerepo=fedora-cisco-openh264)

log INFO "Checking Fedora $SUPPORTED_FEDORA_RELEASE repository metadata"
dnf "${DNF_REPO_ARGS[@]}" -q makecache >/dev/null || die 'DNF metadata/connectivity check failed.'
...
for package in "${packages[@]}"; do
  if rpm -q "$package" >/dev/null 2>&1; then
    mapfile -t installed_records < <(dnf "${DNF_REPO_ARGS[@]}" -q repoquery --installed \
    ...
    for record in "${installed_records[@]}"; do
      ...
      if [[ $origin == '<unknown>' ]]; then
        ...
        mapfile -t available_records < <(dnf "${DNF_REPO_ARGS[@]}" -q repoquery --available \
  ...
  missing+=("$package")
  if ! query_output=$(dnf "${DNF_REPO_ARGS[@]}" -q repoquery --available --latest-limit 1 \
...
if (( ${#missing[@]} > 0 )); then
  ...
  sudo dnf "${DNF_REPO_ARGS[@]}" install --assumeyes --setopt=install_weak_deps=False "${missing[@]}"
...
ensure_managed_link "$ROOT/config/environment.d/20-desktop-theme.conf" "$HOME/.config/environment.d/20-desktop-theme.conf"
ensure_managed_link "$ROOT/config/gtk-3.0" "$HOME/.config/gtk-3.0"
...
zen_binary=${XDG_DATA_HOME:-$HOME/.local/share}/$PROJECT_ID/zen/zen/zen
ensure_managed_link "$zen_binary" "$HOME/.local/bin/zen"
ensure_managed_link "$ROOT/scripts/qs-ipc.sh" "$HOME/.local/bin/qs-ipc.sh"
ensure_managed_link "$ROOT/assets/backgrounds/raiju" "$HOME/.local/share/backgrounds/raiju"
for desktop_file in "$ROOT"/config/applications/*.desktop; do
  ensure_managed_link "$desktop_file" "$HOME/.local/share/applications/$(basename "$desktop_file")"
done
...
systemctl --user enable fedora-sway-clipboard.service fedora-sway-wallpaper.timer
if systemctl --user is-active --quiet fedora-sway-session.target; then
  systemctl --user restart fedora-sway-clipboard.service
  systemctl --user start fedora-sway-wallpaper.timer
fi
